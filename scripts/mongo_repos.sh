...
[ -n "$_MONGO_REPOS_SH_LOADED" ] && return 0
_MONGO_REPOS_SH_LOADED=1

SCRIPT_DIR_MONGO_REPOS="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR_MONGO_REPOS/config.sh"
source "$SCRIPT_DIR_MONGO_REPOS/mongo_store.sh"
...
save_repo_to_mongo() {
    ...
    escaped_repo_url=$(escape_mongo_str "$repo_url")
    escaped_target_path=$(escape_mongo_str "$target_path")
...
update_repo_screen_details_in_mongo() {
    ...
    escaped_repo_name=$(escape_mongo_str "$repo_name")
    escaped_screen_name=$(escape_mongo_str "$screen_name")
    escaped_start_cmd=$(escape_mongo_str "$start_cmd")
